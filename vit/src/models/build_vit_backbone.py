...
from .vit_prompt.vit_BPT import PromptedVisionTransformer_BPT
from .vit_prompt.vit_prompt_VK import PromptedVisionTransformer_Prompt_VK
from .vit_prompt.vit_VFPT import PromptedVisionTransformer_Fourier
...
MODEL_ZOO = {
    ...
    "timm_vitb16_21kft1k": "ViT-B_16_timm_21kft1k.npz",
...
def build_vit_sup_models(
    model_type, crop_size, prompt_cfg=None, model_root=None, adapter_cfg=None, load_pretrain=True, vis=False, p_vk_cfg=None, transfer_type="linear",
):
    ...
    m2featdim = {
        ...
        "timm_vitb16_21kft1k": 768,
    }
    if p_vk_cfg is not None:
        model = PromptedVisionTransformer_Prompt_VK(
            p_vk_cfg, model_type, crop_size, num_classes=-1, vis=vis
        )
    elif transfer_type == "prompt_bilinear" and prompt_cfg is not None:
        model = PromptedVisionTransformer_BPT(
            prompt_cfg, model_type, crop_size, num_classes=-1, vis=vis
        )
    elif transfer_type == "prompt_fourier" and prompt_cfg is not None:
        model = PromptedVisionTransformer_Fourier(
            prompt_cfg, model_type, crop_size, num_classes=-1, vis=vis
        )
