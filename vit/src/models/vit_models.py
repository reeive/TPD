...
class ViT(nn.Module):
    ...
    def __init__(self, cfg, load_pretrain=True, vis=False):
        ...
        if (
            ...
            and "prompt" not in cfg.MODEL.TRANSFER_TYPE
            and cfg.MODEL.TRANSFER_TYPE != "prompt_bilinear"
        ):
    ...
    def build_backbone(self, prompt_cfg, cfg, adapter_cfg, load_pretrain, vis):
        ...
        p_vk_cfg = cfg.MODEL.P_VK if transfer_type == "prompt_vk" else None
        self.enc, self.feat_dim = build_vit_sup_models(
            ...
            p_vk_cfg=p_vk_cfg, transfer_type=transfer_type,
        )
        ...
        elif transfer_type == "prompt" and prompt_cfg is not None and prompt_cfg.LOCATION == "below":
            for k, p in self.enc.named_parameters():
                if "prompt" not in k and "embeddings.patch_embeddings.weight" not in k  and "embeddings.patch_embeddings.bias" not in k:
                    p.requires_grad = False

        elif transfer_type == "prompt_vk":
            # VPT-style "prompt_*" plus E2VPT Q/K/V tunable tensors (deep_QKV_*)
            for k, p in self.enc.named_parameters():
                kl = k.lower()
                if "prompt" not in kl and "qkv" not in kl:
                    p.requires_grad = False

        elif transfer_type == "prompt_fourier":
            # VFPT: prompt tensors + Fourier branch (module name FT)
            for k, p in self.enc.named_parameters():
                kl = k.lower()
                train = "prompt" in kl or "ft.weight" in kl or "ft.bias" in kl
                if not train:
                    p.requires_grad = False

        elif transfer_type == "prompt_bilinear":
            for k, p in self.enc.named_parameters():
                kl = k.lower()
                if ("random_vectors" not in kl) and ("conv1x1" not in kl):
                    p.requires_grad = False
